...
function getLedgerSlice(opts: { 
  ...
  if (range === "since_start") {
    ...
    if (!activeSession) {
      return { error: "No active session found. Use /starstory showtime start to begin one." };
    }
...
export const session = {
  ...
  async execute(interaction: any, ctx: CommandCtx | null) {
    ...
    if (sub === "recap") {
      ...
      if (!session) {
        await interaction.reply({
          content: "No active or ingested session found. Use /starstory showtime start to begin one, or ingest a recording.",
          ephemeral: true,
        });
        return;
      }
